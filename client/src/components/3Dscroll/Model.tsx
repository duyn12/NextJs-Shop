...
export default function Model({scrollProgress}) {
    ...
    const image = useRef();
    const texture = useTexture("/images/vga_ZeroFro_01.png")
    ...
    const { amplitude, waveLength } = useControls({
        amplitude: { value: 0, min: 0, max: 2, step: 0.1 },
        waveLength: { value: 0, min: 0, max: 20, step: 0.5 },
    })
    ...
    useFrame( () => {
       ...
       image.current.material.uniforms.vUvScale.value.set(1, aspectRatio / scaleRatio)
        ...
        image.current.material.uniforms.uTime.value += 0.01;
